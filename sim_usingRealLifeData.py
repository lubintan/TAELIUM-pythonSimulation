...
NAME = "bitcoinTxData%d_%d_%d_%d_%d_%d" % (now.year, now.month, now.day, now.hour, now.minute, now.second)
...
# dailyInterestPaidOut
    ...
    for i in range(STARTING_NUM_USERS):
        ...
        userList.append((supplyCurrent * 0.6) / STARTING_NUM_USERS)
    ...
    while dayCounter < DAYS:
        ...
        sys.stdout.write("\rCalculating... %.2f %% complete. Currently on day %i. No. of Users: %i. No. of Txes (Today only): %i." % (float(dayCounter)*100.0/(DAYS-1), dayCounter, len(userList), numTxList[dayCounter]))
        sys.stdout.flush()
...
#######################################################################################################
#######################   R EQUATION!!!!##############################################################
    dailyVolumeOnline = avgHoldings[dayCounter]
...
def blockOperations(userList):
    ...
    numberOfTransactionsThisBlock = numTxList[dayCounter] / NUM_OF_DAILY_BLOCKS
